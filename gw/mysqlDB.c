...
int search_smpp_user(Octstr *system_id, Octstr *password, Octstr *system_type)
{
	Octstr *sql;
	MYSQL_RES *res;
	MYSQL_ROW row;
	sql = octstr_format(SQL_SELECT_SMPP_USERS, octstr_get_cstr(system_id), octstr_get_cstr(password), octstr_get_cstr(system_type));
...
void mysql_save_msg(Msg *msg, Octstr *momt /*, Octstr smsbox_id */)
{
	...
	sql = octstr_format(SQLBOX_MYSQL_INSERT_QUERY, sqlbox_logtable, st_str(momt), st_str(msg->sms.sender), st_str(msg->sms.receiver), st_str(msg->sms.udhdata), st_str(msg->sms.msgdata),
			st_num(msg->sms.time), st_str(msg->sms.smsc_id), st_str(msg->sms.service), st_str(msg->sms.account), st_num(msg->sms.sms_type), st_num(msg->sms.mclass), st_num(msg->sms.mwi),
			st_num(msg->sms.coding), st_num(msg->sms.compress), st_num(msg->sms.validity), st_num(msg->sms.deferred), st_num(msg->sms.dlr_mask), st_str(msg->sms.dlr_url), st_num(msg->sms.pid),
			st_num(msg->sms.alt_dcs), st_num(msg->sms.rpi), st_str(msg->sms.charset), st_str(msg->sms.boxc_id), st_str(msg->sms.binfo), st_str(msg->sms.meta_data));
	sql_update(sql);
	while (stuffcount > 0)
	{
